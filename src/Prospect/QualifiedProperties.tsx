...
interface QualifiedPropertiesProps {
  ...
  onCompareUnits?: (
    units: { property: QualifiedProperty; unit: Unit }[]
  ) => void;
  applicantData: {
    unitType: string;
...
// No static data - all data comes from Firebase
...
const QualifiedProperties: React.FC<QualifiedPropertiesProps> = ({
  ...
  const [selectedBedroomTypes, setSelectedBedroomTypes] = useState<string[]>([
    "1",
    "2",
    "3",
  ]);
  const [comparisonUnits, setComparisonUnits] = useState<
    { property: QualifiedProperty; unit: Unit }[]
  >(initialComparisonUnits);
  const [showFilters, setShowFilters] = useState(false);
  const [showUnitDetails, setShowUnitDetails] = useState(false);
  const [selectedUnitForDetails, setSelectedUnitForDetails] = useState<{
    property: QualifiedProperty;
    unit: Unit;
  } | null>(null);
  const [selectedPropertyForTour, setSelectedPropertyForTour] =
    useState<QualifiedProperty | null>(null);
  const [selectedUnitForTour, setSelectedUnitForTour] = useState<Unit | null>(
    null
  );
  ...
  const loadUnitsData = async () => {
    try {
      const unitsQuery = query(collection(db, "units"), limit(100));
  ...
  const transformFirebaseToQualifiedProperties = async (
    firebaseData: any[]
    ...
    return firebaseData.map((property, index) => {
      ...
      return {
        ...
        units: propertyUnits, // Only show units that actually exist in Firebase
      };
    });
  };
      ...
      if (selectedProperty && unitsData.length > 0) {
        ...
        try {
          const transformedProperties =
            await transformFirebaseToQualifiedProperties(dynamicProperties);
        ...
        console.log("No dynamic data available - showing empty state");
        setQualifiedProperties([]);
      }
    };
  ...
  const getFilteredProperties = () => {
    console.log(
      "getFilteredProperties called with qualifiedProperties:",
      qualifiedProperties
    );
  ...
  const getFilteredUnits = (property: QualifiedProperty) => {
    ...
    const filtered = property.units.filter((unit) => {
      ...
      const bedroomMatch =
        selectedBedroomTypes.includes(unit.bedrooms.toString()) ||
      ...
      if (unit.leaseTerms && unit.leaseTerms.length > 0) {
        ...
        if (
          maxRent < searchFilters.minPrice ||
          minRent > searchFilters.maxPrice
        ) {
  ...
  const totalAvailableUnits = filteredProperties.reduce(
    (sum, property) => sum + getFilteredUnits(property).length,
    0
  );

  const totalQualifiedUnits = filteredProperties.reduce(
    (sum, property) =>
      sum + getFilteredUnits(property).filter((unit) => unit.qualified).length,
    0
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 ">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm">
                {selectedProperty
                  ? `Showing ${totalQualifiedUnits} available units for ${
          ...
          >
            <Heart
              className={`h-5 w-5 mr-2 ${
                comparisonUnits.length > 0 ? "fill-current" : ""
              }`}
            />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="bg-white/80 backdrop-blur-md  backdrop-blur-md rounded-2xl shadow-xl p-6 mb-8 border border-green-200/50"
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4">
              ...
              <p className="font-bold text-gray-900">
                {applicantData.desiredTourDate || "Flexible"}
              </p>
            </div>
            <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4">
              <p className="font-medium text-gray-600 mb-1">Move-in Date</p>
              <p className="font-bold text-gray-900">
                {applicantData.moveInDate || "Flexible"}
              </p>
            </div>
            <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4">
              <p className="font-medium text-gray-600 mb-1">Lease Term</p>
              <p className="font-bold text-gray-900">
                {applicantData.desiredLeaseTerm
                  ? `${applicantData.desiredLeaseTerm} months`
                  : "Flexible"}
              </p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-sm text-gray-600">
              <span className="font-semibold">Property Stats:</span>{" "}
              {qualifiedProperties.length} properties •{" "}
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      value={
                        searchFilters.petFriendly === null
                          ? "any"
                          : searchFilters.petFriendly.toString()
                      }
                      onChange={(e) =>
                        setSearchFilters((prev) => ({
                          ...prev,
                            ...
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() =>
                                    toggleComparisonUnit(property, unit)
                                  }
                            ...
                            <div className="space-y-2">
                              ...
                              {unit.leaseTerms.length > 2 && (
                                <div className="text-center text-sm text-gray-500">
                                  +{unit.leaseTerms.length - 2} more lease
                                  options
                                </div>
                              )}
                            </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex gap-2">
                                ...
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.98 }}
                                  onClick={() =>
                                    handleUnitDetailsClick(property, unit)
                                  }
                                  className="flex-1 border hover:bg-blue-600 hover:text-white border-gray-200 text-gray-600 text-sm px-4 py-2 font-semibold rounded-lg transition-all duration-200"
                                >
